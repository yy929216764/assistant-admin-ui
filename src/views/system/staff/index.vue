    ...
    <el-col :span="4" :xs="24">
      <ContentWrap class="h-1/1">
        <div class="dept-tree-header">
          <span class="font-bold">机构部门</span>
        </div>
        <el-tree
          ref="treeRef"
          :data="deptList"
        ...
        >
          ...
          <el-form-item>
            ...
            <el-button
              ...
              v-hasPermi="['system:staff:create']"
            >
              <Icon icon="ep:plus" /> 新增人员
            </el-button>
          </el-form-item>
        </el-form>
      </ContentWrap>
      <ContentWrap>
        <el-table v-loading="loading" :data="filteredList" @selection-change="handleRowCheckboxChange">
          ...
          <el-table-column label="岗位" align="center" width="120">
            <template #default="scope">
              <el-tag v-if="scope.row.postNames && scope.row.postNames.length > 0" size="small" type="success">
                {{ scope.row.postNames[0] }}
              </el-tag>
              <span v-else-if="scope.row.roleNames && scope.row.roleNames.length > 0" class="text-gray-400 text-xs">
          ...
          <el-table-column label="状态" key="status" align="center" width="80">
            <template #default="scope">
              <el-switch
                ...
                :disabled="!checkPermi(['system:staff:update'])"
              />
            </template>
          </el-table-column>
          <el-table-column
            ...
            <template #default="scope">
              <div class="flex items-center justify-center">
                ...
                <el-dropdown
                  ...
                  v-hasPermi="['system:staff:delete', 'system:staff:update-password', 'system:permission:assign-user-role']"
                >
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item
                        ...
                        v-if="checkPermi(['system:staff:delete'])"
                      >
                        <Icon icon="ep:delete" />删除
                      </el-dropdown-item>
                      <el-dropdown-item
                        command="handleResetPwd"
                        v-if="checkPermi(['system:staff:update-password'])"
                      >
...
import UserAssignRoleForm from '../user/UserAssignRoleForm.vue'
...
const filteredList = computed(() => {
  return list.value.filter(user => {
    // 如果用户角色包含学生角色，则过滤掉
    if (user.roleIds && user.roleIds.includes(STUDENT_ROLE_ID)) {
      return false
    }
    return true
  })
